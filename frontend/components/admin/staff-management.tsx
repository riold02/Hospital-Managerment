...
export function StaffManagement() {
  ...
  const loadStaffData = async () => {
    try {
      ...
      // Filter nurses and workers
      const nursesData = staffData.filter((s: Staff) => s.role === "Nurse")
      const workersData = staffData.filter((s: Staff) => s.role === "Worker")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingStaff) {
        // Update staff
        await apiClient.updateStaff(editingStaff.staff_id, {
        ...
        // Refresh staff list
        const updatedStaff = await apiClient.getStaff()
        setStaff(updatedStaff)
        toast({ title: "Thành công", description: "Cập nhật nhân viên thành công!" })
      } else {
        // Create new staff via register API (Admin only)
        await apiClient.registerStaff({
          email: formData.email,
